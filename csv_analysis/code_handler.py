...
def validate_code(state:AgentState) -> AgentState:
    try:
        ...
        state['python_code'] = cleanResponse(response)
...
def cleanResponse(response:str):
    """
    Cleans the response from the LLM by removing any unwanted characters or formatting.
    """
    # Remove unwanted characters or formatting
    cleaned_response = response.removeprefix('```python\n').removesuffix('\n```')
