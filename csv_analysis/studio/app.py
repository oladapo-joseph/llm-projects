...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        st.dataframe(st.session_state.agent_state["df"].head())

# Question input
def get_question(prompt):
    st.session_state.agent_state['question'] = prompt
    if st.session_state.agent_state['question']:
        st.session_state.agent_state['next_state'] = "generate_python_code"
        
# Workflow execution
def startWorkflow():
    if st.session_state.agent_state['df'] is not None:
        # Initialize the graph and start the workflow
        while st.session_state.agent_state['next_state'] != "end":
